...
pub enum DeviceType {
    /// CPU device
    Cpu = quill_mlx_sys::mlx_device_type__MLX_CPU,

    /// GPU device
    Gpu = quill_mlx_sys::mlx_device_type__MLX_GPU,
}

/// Representation of a Device in MLX.
pub struct Device {
    pub(crate) c_device: quill_mlx_sys::mlx_device,
}

impl PartialEq for Device {
    fn eq(&self, other: &Self) -> bool {
        unsafe { quill_mlx_sys::mlx_device_equal(self.c_device, other.c_device) }
    }
}
...
impl Device {
    ...
    pub fn new(device_type: DeviceType, index: i32) -> Device {
        let c_device = unsafe { quill_mlx_sys::mlx_device_new_type(device_type.into(), index) };
        Device { c_device }
    }

    /// Try to get the default device.
    pub fn try_default() -> Result<Self> {
        Device::try_from_op(|res| unsafe { quill_mlx_sys::mlx_get_default_device(res) })
    }
    ...
    pub fn get_index(&self) -> Result<i32> {
        i32::try_from_op(|res| unsafe { quill_mlx_sys::mlx_device_get_index(res, self.c_device) })
    }

    /// Get the device type
    pub fn get_type(&self) -> Result<DeviceType> {
        DeviceType::try_from_op(|res| unsafe { quill_mlx_sys::mlx_device_get_type(res, self.c_device) })
    }
...
impl Drop for Device {
    fn drop(&mut self) {
        let status = unsafe { quill_mlx_sys::mlx_device_free(self.c_device) };
